...
方針: 関係節の対は **前半の出来事(誰が誰に)が2枚とも同じ** で、後半の状態にある人物だけが違う(前半まで入れ替わると語順レベルと区別がつかない)。10組20枚。前半まで入れ替わっていた4組(たたいた・ぶつかった・傘を貸した・押されている)は単文レベルに残してある。

- [ ] ST に「関係節と語順の違いが出ているか」「絵の手がかりが実機の大きさで十分か」を確認。難易度は語順より一段上なので、まず軽度〜中等度の方で
