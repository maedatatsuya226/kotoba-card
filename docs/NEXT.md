...
### 3b. 関係節レベルの整理 — **Windows側の確認・本番反映待ち**(`next` v37)
...
- 差し替え4組(8枚) + 作り直し2枚は生成・最適化・取り込み済み
- [x] Mac で追加分26件(単文8・関係節18)をVOICEVOXで生成。全238カードのWAV形式・欠損・破損を検査済み
...
- 情景76枚(単文16・意味12・語順14・助詞10・関係節24)、音声も全件同梱。カードは `image` で別カードの絵を流用でき(受動文6件)、`pairs`(配列)で対の絵を複数指定できる
